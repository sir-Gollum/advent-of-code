...
class Grid:
    def __init__(self, g: str):
        self.g = [list(line) for line in g.splitlines()]
        self.num_rows = len(self.g)
        self.num_cols = len(self.g[0])

    def __str__(self):
        return '\n'.join([''.join(line) for line in self.g])

    def move(self):
        new = copy.deepcopy(self.g)
        ...
        for row_idx in range(self.num_rows):
            for col_idx in range(self.num_cols):
                if self.g[row_idx][col_idx] != '>':
                    continue

                target_col_idx = (col_idx + 1) % self.num_cols
                if self.g[row_idx][target_col_idx] != '.':
                    continue

                # move
                new[row_idx][col_idx] = '.'
                new[row_idx][target_col_idx] = '>'
                num_moves += 1
        ...
        for row_idx in range(self.num_rows):
            for col_idx in range(self.num_cols):
                if new[row_idx][col_idx] != 'v':
                    continue

                target_row_idx = (row_idx + 1) % self.num_rows
                if new[target_row_idx][col_idx] != '.':
                    continue

                # move
                new2[row_idx][col_idx] = '.'
                new2[target_row_idx][col_idx] = 'v'
                num_moves += 1

        return Grid('\n'.join([''.join(line) for line in new2])), num_moves
...
....v..v.>    
""".strip()
    assert str(g) == expected, f'Expected:\n{expected}\nGot:\n{str(g)}'
...
def solve1(data):
    ...
    steps = 0
    while True:
        grid, moves = grid.move()
        steps += 1
        if not moves:
            print(f'Grid is not moving after {steps} steps')
            return
